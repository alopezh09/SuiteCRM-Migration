declare function addToValidateCallback(formname: string, name: string, type: string, isRequired: boolean, msg: string, callback: () => boolean): void
declare function removeFromValidate(formname: string, name: string): void
...
interface FormDef {
	[key: string]: { field: Field, dynamics?: { [val: string]: string[] } }
}

let addRequired = (field: Field): void => {
	...
	if ($(`[field="${field.name}"]`).parent().find('.label>.required').length == 0) {
		...
		let requiredSpan: HTMLElement = $('<span/>', {
			class: 'required',
			html: '*'
		})[0]
...
let removeRequired = (field: Field): void => {
...
let hide = (field: Field): void => {
	$(`[field="${field.name}"]`).parent().addClass('hidden');
}

let show = (field: Field): void => {
	$(`[field="${field.name}"]`).parent().removeClass('hidden')
}

let initForm = (formData: FormDef): void => {
...
let watchForm = (formData: FormDef): void => {
	let fieldNames = Object.keys(formData);
...
let form: any = JSON.parse(JSON.stringify(formReq));
