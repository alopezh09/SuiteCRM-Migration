...
class SOELHooksLeads
{
    ...
    function pre_save( &$bean, $event, $arguments )
    {
        ...
        if ( empty( $prospecto_encontrado->id ) ) {
            ...
            if (isset( $lead)) {
                //$this->update_return_and_exit( $lead ); -> Codigo comentado, la siguiente condicion estaba en la version de VETA
    ...
    // private function obtener_comercial_acumulado( $bean, $list )
    // {
    //     $uid   = null;
    //     $lista = array();

    //     $q = "SELECT DISTINCT related_id, COUNT(leads.id) AS CONT
    ...
    //                 LEFT JOIN leads ON leads.assigned_user_id = users.id AND leads.deleted = 0 AND leads.status = 'Asignado'
    ...
    {
		error_log("LEADS_ERROR post save: " . print_r($bean, true));
        ...
        if (isset($bean->requerimiento)) {
            ...
            $requirement->virtual_lead_id = $bean->id;
			
			error_log("CACHE - VISA EXPIRY DATE - requerimiento ". $requirement->visa_app_exp_date);
			
			$datetime = $bean->fecha_expiracion_visa_c; 
			$date = date('Y-m-d', strtotime($datetime));
			$requirement->visa_app_exp_date = $date;


			//$requirement->visa_app_exp_date = $bean->fecha_expiracion_visa_c;
			
			error_log("CACHE - VISA EXPIRY DATE - lead ". $bean->fecha_expiracion_visa_c);
		...
		//codigo para crear el lead como compañia si viene el parametro is b2b
		
		if ($bean->is_b2b_c == 1){
			error_log("COMPANY, es una compañia que se debe crear en el crm entro a 1");
		}
		if ($bean->is_b2b_c){
			error_log("COMPANY, es una compañia que se debe crear en el crm entro a TRUE"); 
		}
		...
		if(isset($bean->id)){
			//error_log( "prospecto - " . $_REQUEST['return_module']);
			if($_REQUEST['return_module'] == 'Leads'){
				//error_log( "prospecto - " . print_r($_REQUEST, true) );
				...
				foreach ($ctasCobro as $ctaCobro) {
					...
					foreach ($procesosVenta as $workflow) {
						
						//if($workflow->name == '233'){
							//$workflow->virtual_visa_exp_date_c = $bean->fecha_expiracion_visa_c;
							
							$bean->db->query("UPDATE opportunities_cstm set 											
												virtual_visa_exp_date_c = '" . substr($bean->fecha_expiracion_visa_c, 0, 10) . "',
												visa_expiration_text_c = '" . $bean->expiracion_visa_c . "'
											where id_c='" . $workflow->id . "'");	
                ...
                foreach ($requerimientos as $requerimiento) {                    
					error_log( "prospecto - requerimientos " . $requerimiento->id);
        ...
        if (!empty($bean->contact_id)) {
            ...
            $c->first_name = $bean->first_name;
            $c->last_name = $bean->last_name;
            $c->phone_mobile = $bean->phone_mobile;

            $c->phone_home = $bean->phone_home;
            $c->pasaporte_c = $bean->pasaporte_c;
            $c->visa_c = $bean->visa_c;

            $c->pais_c = $bean->pais_c;
            $c->departamento_c = $bean->departamento_c;
            $c->ciudad_c = $bean->ciudad_c;
            $c->email1 = $bean->email1;
