<?php
$module_name = 'Veta_Requerimiento';
$viewdefs [$module_name] = 
array (
  'DetailView' => 
  array (
    'templateMeta' => 
    array (
      'form' => 
      array (
        'buttons' => 
        array (
          0 => 'EDIT',
          1 => 'DUPLICATE',
          2 => 'DELETE',
          3 => 
          array (
            'customCode' => '<input value="Send Master Plan" type="button" class="button" onClick="document.location=\'index.php?module=Veta_Requerimiento&action=send_masterplan&pid=\'+ document.getElementsByName(\'record\')[0].value" >',
          ),
          4 => 'FIND_DUPLICATES',
        ),
      ),
      'maxColumns' => '2',
      'widths' => 
      array (
        0 => 
        array (
          'label' => '10',
          'field' => '30',
        ),
        1 => 
        array (
          'label' => '10',
          'field' => '30',
        ),
      ),
      'useTabs' => true,
      'tabDefs' => 
      array (
        'DEFAULT' => 
        array (
          'newTab' => true,
          'panelDefault' => 'expanded',
        ),
        'LBL_DETAILVIEW_PANEL7' => 
        array (
          'newTab' => false,
          'panelDefault' => 'collapsed',
        ),
        'LBL_DETAILVIEW_PANEL6' => 
        array (
          'newTab' => false,
          'panelDefault' => 'collapsed',
        ),
        'LBL_DETAILVIEW_PANEL2' => 
        array (
          'newTab' => false,
          'panelDefault' => 'collapsed',
        ),
        'LBL_DETAILVIEW_PANEL1' => 
        array (
          'newTab' => false,
          'panelDefault' => 'collapsed',
        ),
        'LBL_DETAILVIEW_PANEL4' => 
        array (
          'newTab' => false,
          'panelDefault' => 'collapsed',
        ),
        'LBL_DETAILVIEW_PANEL9' => 
        array (
          'newTab' => true,
          'panelDefault' => 'expanded',
        ),
      ),
    ),
    'panels' => 
    array (
      'default' => 
      array (
        0 => 
        array (
          0 => 'name',
          1 => 
          array (
            'name' => 'from_a_company_c',
            'label' => 'LBL_FROM_A_COMPANY',
          ),
        ),
        1 => 
        array (
          0 => 
          array (
            'name' => 'de_posventa_c',
            'label' => 'LBL_DE_POSVENTA_C',
          ),
          1 => 
          array (
            'name' => 'previous_workflow_c',
            'studio' => 'visible',
            'label' => 'LBL_PREVIOUS_WORKFLOW',
          ),
        ),
        2 => 
        array (
          0 => 'assigned_user_name',
          1 => 
          array (
            'name' => 'referido',
            'label' => 'LBL_REFERIDO',
            'customCode' => '{if $fields.referido.value}{$fields.referido.value}{else}{if $fields.campana.value}{$fields.campana.value}{/if}{/if}',
          ),
        ),
        3 => 
        array (
          0 => 
          array (
            'name' => 'veta_requerimiento_contacts_name',
            'label' => 'LBL_VETA_REQUERIMIENTO_CONTACTS_FROM_CONTACTS_TITLE',
          ),
          1 => 
          array (
            'name' => 'veta_requerimiento_leads_name',
            'label' => 'LBL_VETA_REQUERIMIENTO_LEADS_FROM_LEADS_TITLE',
          ),
        ),
        4 => 
        array (
          0 => 
          array (
            'name' => 'estado_semaforizacion_c',
            'studio' => 'visible',
            'label' => 'LBL_ESTADO_SEMAFORIZACION',
            'customCode' => '<div style="width: 20px;height: 20px;background: {$fields.estado_semaforizacion_c.value};border-radius: 50%;margin: auto;"></div>',
          ),
          1 => 
          array (
            'name' => 'detalle_semaforizacion_c',
            'studio' => 'visible',
            'label' => 'LBL_DETALLE_SEMAFORIZACION',
          ),
        ),
        5 => 
        array (
          0 => 
          array (
            'name' => 'campana',
            'studio' => 'visible',
            'label' => 'LBL_CAMPANA',
          ),
          1 => 
          array (
            'name' => 'fuente',
            'studio' => 'visible',
            'label' => 'LBL_FUENTE',
          ),
        ),
        6 => 
        array (
          0 => 
          array (
            'name' => 'fecha_proximo_contacto',
            'label' => 'LBL_FECHA_PROXIMO_CONTACTO',
          ),
          1 => 
          array (
            'name' => 'fecha_ultimo_contacto',
            'label' => 'LBL_FECHA_ULTIMO_CONTACTO',
          ),
        ),
        7 => 
        array (
          0 => 
          array (
            'name' => 'fecha_viaje_txt',
            'label' => 'LBL_FECHA_VIAJE_TXT',
          ),
          1 => 
          array (
            'name' => 'fecha_viaje',
            'label' => 'LBL_FECHA_VIAJE',
          ),
        ),
        8 => 
        array (
          0 => 
          array (
            'name' => 'estado',
            'studio' => 'visible',
            'label' => 'LBL_ESTADO',
          ),
          1 => 
          array (
            'name' => 'fecha_primer_presupuesto',
            'label' => 'LBL_FECHA_PRIMER_PRESUPUESTO',
          ),
        ),
        9 => 
        array (
          0 => 
          array (
            'name' => 'company_stage_c',
            'studio' => 'visible',
            'label' => 'LBL_COMPANY_STAGE',
          ),
          1 => 
          array (
            'name' => 'presupuesto',
            'label' => 'LBL_PRESUPUESTO',
          ),
        ),
        10 => 
        array (
          0 => 
          array (
            'name' => 'visto_bueno_comercial',
            'label' => 'LBL_VISTO_BUENO_COMERCIAL',
          ),
          1 => 
          array (
            'name' => 'visto_bueno_visas',
            'label' => 'LBL_VISTO_BUENO_VISAS',
          ),
        ),
        11 => 
        array (
          0 => 
          array (
            'name' => 'date_entered',
            'customCode' => '{$fields.date_entered.value} {$APP.LBL_BY} {$fields.created_by_name.value}',
            'label' => 'LBL_DATE_ENTERED',
          ),
          1 => 
          array (
            'name' => 'date_modified',
            'customCode' => '{$fields.date_modified.value} {$APP.LBL_BY} {$fields.modified_by_name.value}',
            'label' => 'LBL_DATE_MODIFIED',
          ),
        ),
        12 => 
        array (
          0 => 
          array (
            'name' => 'virtual_all_notes',
            'studio' => 'visible',
            'label' => 'LBL_VIRTUAL_NOTE',
          ),
        ),
        13 => 
        array (
          0 => 
          array (
            'name' => 'url_c',
            'label' => 'LBL_URL',
          ),
        ),
        14 => 
        array (
          0 => 
          array (
            'name' => 'TEL_CASA',
            'label' => 'LBL_TEL_CASA',
            'customCode' => '{$TELCASA}',
          ),
          1 => 
          array (
            'name' => 'CELULAR',
            'label' => 'LBL_CELULAR',
            'customCode' => '{$CELULAR}',
          ),
        ),
        15 => 
        array (
          0 => 
          array (
            'name' => 'PASAPORTE',
            'label' => 'LBL_PASAPORTE',
            'customCode' => '{$PASAPORTE}',
          ),
          1 => 
          array (
            'name' => 'EMAIL',
            'label' => 'LBL_EMAIL',
            'customCode' => '{$EMAIL}',
          ),
        ),
        16 => 
        array (
          0 => 
          array (
            'name' => 'workplace_location_c',
            'label' => 'LBL_WORKPLACE_LOCATION',
          ),
          1 => 
          array (
            'name' => 'consultation_date',
            'label' => 'LBL_CONSULTATION_DATE',
          ),
        ),
        17 => 
        array (
          0 => 
          array (
            'name' => 'aplicant_1st_payment_date',
            'label' => 'LBL_APPLICANT_1ST_PAYMENT_DATE',
          ),
          1 => 
          array (
            'name' => 'aplicant_first_payment_amount',
            'label' => 'LBL_APPLICANT_FIRST_PAYMENT_AMOUNT',
          ),
        ),
        18 => 
        array (
          0 => 
          array (
            'name' => 'profession',
            'label' => 'LBL_PROFESSION',
          ),
          1 => 
          array (
            'name' => 'current_visa_subclass',
            'label' => 'LBL_CURRENT_VISA_SUBCLASS',
          ),
        ),
        19 => 
        array (
          0 => 
          array (
            'name' => 'current_job_position',
            'label' => 'LBL_CURRENT_JOB_POSITION',
          ),
          1 => 
          array (
            'name' => 'month_of_experience',
            'label' => 'LBL_MONTHS_OF_EXPERIENCEL',
          ),
        ),
        20 => 
        array (
          0 => 
          array (
            'name' => 'level_of_english',
            'label' => 'LBL_LEVEL_OF_ENGLISH',
          ),
          1 => 
          array (
            'name' => 'nationality',
            'label' => 'LBL_NATIONALITY',
          ),
        ),
        21 => 
        array (
          0 => 
          array (
            'name' => 'potential_visa_subclass',
            'label' => 'LBL_POTENTIAL_VISA_SUBCLASS',
          ),
          1 => 
          array (
            'name' => 'cost_agrement_visa_subclass',
            'label' => 'LBL_COST_AGREMENT_NUMBER',
          ),
        ),
        22 => 
        array (
          0 => 
          array (
            'name' => 'cost_agrement_number',
            'label' => 'LBL_COST_AGREMENT_NUMBER',
          ),
          1 => 
          array (
            'name' => 'quote_date',
            'label' => 'LBL_QUOTE_DATE',
          ),
        ),
        23 => 
        array (
          0 => 
          array (
            'name' => 'leap_id',
            'label' => 'LBL_LEAP_ID',
          ),
        ),
        24 => 
        array (
          0 => 
          array (
            'name' => 'migration_manager_user_c',
            'label' => 'LBL_MIGRATION_MANAGER_USER',
          ),
          1 => 
          array (
            'name' => 'migration_manager_password_c',
            'label' => 'LBL_MIGRATION_MANAGER_PASSWORD',
          ),
        ),
        25 => 
        array (
          0 => 
          array (
            'name' => 'recluter_name',
            'label' => 'LBL_RECLUTER_NAME',
          ),
          1 => 
          array (
            'name' => 'company_name',
            'label' => 'LBL_COMPANY_NAME',
          ),
        ),
        26 => 
        array (
          0 => 
          array (
            'name' => 'industry_aplicant',
            'label' => 'LBL_INDUSTRY_APLICANT',
          ),
          1 => 
          array (
            'name' => 'company_first_payment_amount',
            'label' => 'LBL_COMPANY_FIRST_PAYMENT_AMOUNT',
          ),
        ),
        27 => 
        array (
          0 => 
          array (
            'name' => 'company_1st_payment_date',
            'label' => 'LBL_COMPANY_1ST_PAYMENT_DATE',
          ),
        ),
        28 => 
        array (
          0 => 
          array (
            'name' => 'ocupation',
            'label' => 'LBL_OCUPATION',
          ),
        ),
        29 => 
        array (
          0 => 
          array (
            'name' => 'nomination_app_exp_date',
            'label' => 'LBL_NOMINATION_APP_EXP_DATE',
          ),
          1 => 
          array (
            'name' => 'nomination_app_date',
            'label' => 'LBL_NOMINATION_APP_DATE',
          ),
        ),
        30 => 
        array (
          0 => 
          array (
            'name' => 'nomination_approval_date_c',
            'label' => 'LBL_NOMINATION_APPROVAL_DATE',
          ),
          1 => 
          array (
            'name' => 'skill_assessment_app_exp_date',
            'label' => 'LBL_SKILL_ASSESSMENT_APP_EXP_DATE',
          ),
        ),
        31 => 
        array (
          0 => 
          array (
            'name' => 'labour_application_date_c',
            'label' => 'LBL_LABOUR_APPLICATION_DATE',
          ),
          1 => 
          array (
            'name' => 'labour_expectation_date_c',
            'label' => 'LBL_LABOUR_EXPECTATION_DATE',
          ),
        ),
        32 => 
        array (
          0 => 
          array (
            'name' => 'skill_assessment_app_date',
            'label' => 'LBL_SKILL_ASSESSMENT_APP_DATE',
          ),
          1 => 
          array (
            'name' => 'visa_app_exp_date',
            'label' => 'LBL_VISA_APP_EXP_DATE',
          ),
        ),
        33 => 
        array (
          0 => 
          array (
            'name' => 'tbs_application_date_c',
            'label' => 'LBL_TBS_APPLICATION_DATE',
          ),
          1 => 
          array (
            'name' => 'tbs_expectation_date_c',
            'label' => 'LBL_TBS_EXPECTATION_DATE',
          ),
        ),
        34 => 
        array (
          0 => 
          array (
            'name' => 'tbs_approval_date_c',
            'label' => 'LBL_TBS_APPROVAL_DATE',
          ),
          1 => '',
        ),
        35 => 
        array (
          0 => 
          array (
            'name' => 'eoi_application_date_c',
            'label' => 'LBL_EOI_APPLICATION_DATE',
          ),
          1 => 
          array (
            'name' => 'eoi_expectation_date_c',
            'label' => 'LBL_EOI_EXPECTATION_DATE',
          ),
        ),
        36 => 
        array (
          0 => 
          array (
            'name' => 'eoi_appoval_date_c',
            'label' => 'LBL_EOI_APPOVAL_DATE',
          ),
          1 => '',
        ),
        37 => 
        array (
          0 => 
          array (
            'name' => 'rcb_application_date_c',
            'label' => 'LBL_RCB_APPLICATION_DATE',
          ),
          1 => 
          array (
            'name' => 'rcb_expectation_date_c',
            'label' => 'LBL_RCB_EXPECTATION_DATE',
          ),
        ),
        38 => 
        array (
          0 => 
          array (
            'name' => 'rcb_approval_date_c',
            'label' => 'LBL_RCB_APPROVAL_DATE',
          ),
          1 => '',
        ),
        39 => 
        array (
          0 => 
          array (
            'name' => 'roi_application_date_c',
            'label' => 'LBL_ROI_APPLICATION_DATE',
          ),
          1 => 
          array (
            'name' => 'roi_expectation_date_c',
            'label' => 'LBL_ROI_EXPECTATION_DATE',
          ),
        ),
        40 => 
        array (
          0 => 
          array (
            'name' => 'roi_approval_date_c',
            'label' => 'LBL_ROI_APPROVAL_DATE',
          ),
          1 => '',
        ),
        41 => 
        array (
          0 => 
          array (
            'name' => 'visa_app_date',
            'label' => 'LBL_VISA_APP_DATE',
          ),
          1 => 
          array (
            'name' => 'recruitment',
            'label' => 'LBL_RECRUITMENT',
          ),
        ),
        42 => 
        array (
          0 => 
          array (
            'name' => 'recruitment_exp_date',
            'label' => 'LBL_RECRUITMENT_EXP_DATE',
          ),
        ),
        43 => 
        array (
          0 => 
          array (
            'name' => 'transferred_from_billing_c',
            'studio' => 'visible',
            'label' => 'LBL_TRANSFERRED_FROM_BILLING',
          ),
          1 => '',
        ),
      ),
      'lbl_detailview_panel7' => 
      array (
        0 => 
        array (
          0 => 
          array (
            'name' => 'sponsored_australian_citizen_c',
            'label' => 'LBL_SPONSORED_AUSTRALIAN_CITIZEN',
          ),
          1 => 
          array (
            'name' => 'australian_sponsor_name_c',
            'label' => 'LBL_AUSTRALIAN_SPONSOR_NAME',
          ),
        ),
      ),
      'lbl_detailview_panel6' => 
      array (
        0 => 
        array (
          0 => 
          array (
            'name' => 'main_aplicant_name',
            'label' => 'LBL_MAIN_APLICANT_NAME',
          ),
        ),
        1 => 
        array (
          0 => 
          array (
            'name' => 'secondary_aplicant_name',
            'label' => 'LBL_SECONDARY_APLICANT_NAME',
          ),
          1 => 
          array (
            'name' => 'visa_expire_secondary_applicant_date_c',
            'label' => 'LBL_VISA_EXPIRE_SECONDARY_APPLICANT_DATE',
          ),
        ),
        2 => 
        array (
          0 => 
          array (
            'name' => 'secondary_dob',
            'label' => 'LBL_SECONDARY_DOB',
          ),
          1 => 
          array (
            'name' => 'secondary_pasport_number',
            'label' => 'LBL_SECONDARY_PASPORT_NUMBER',
          ),
        ),
        3 => 
        array (
          0 => 
          array (
            'name' => 'dependent_name',
            'label' => 'LBL_DEPENDENT_NAME',
          ),
          1 => 
          array (
            'name' => 'visa_expire_1st_dependent_date_c',
            'label' => 'LBL_VISA_EXPIRE_1ST_DEPENDENT_DATE',
          ),
        ),
        4 => 
        array (
          0 => 
          array (
            'name' => 'dependent_dob',
            'label' => 'LBL_DEPENDENT_DOB',
          ),
        ),
        5 => 
        array (
          0 => 
          array (
            'name' => 'second_dependent_name',
            'label' => 'LBL_SECOND_DEPENDENT_NAME',
          ),
          1 => 
          array (
            'name' => 'visa_expire_2nd_dependent_date_c',
            'label' => 'LBL_VISA_EXPIRE_2ND_DEPENDENT_DATE',
          ),
        ),
        6 => 
        array (
          0 => 
          array (
            'name' => 'second_dependent_dob',
            'label' => 'LBL_SECOND_DEPENDENT_DOB',
          ),
        ),
        7 => 
        array (
          0 => 
          array (
            'name' => 'third_dependent_name',
            'label' => 'LBL_THIRD_DEPENDENT_NAME',
          ),
          1 => 
          array (
            'name' => 'visa_expire_3rd_dependent_date_c',
            'label' => 'LBL_VISA_EXPIRE_3RD_DEPENDENT_DATE',
          ),
        ),
        8 => 
        array (
          0 => 
          array (
            'name' => 'third_dependent_dob_c',
            'label' => 'LBL_THIRD_DEPENDENT_DOB',
          ),
          1 => '',
        ),
        9 => 
        array (
          0 => 
          array (
            'name' => 'fourth_dependent_name_c',
            'label' => 'LBL_FOURTH_DEPENDENT_NAME',
          ),
          1 => 
          array (
            'name' => 'visa_expire_4th_dependent_date_c',
            'label' => 'LBL_VISA_EXPIRE_4TH_DEPENDENT_DATE_C',
          ),
        ),
        10 => 
        array (
          0 => 
          array (
            'name' => 'fourth_dependent_dob_c',
            'label' => 'LBL_FOURTH_DEPENDENT_DOB',
          ),
          1 => '',
        ),
        11 => 
        array (
          0 => 
          array (
            'name' => 'TEL_CASA',
            'label' => 'LBL_TEL_CASA',
            'customCode' => '{$TELCASA}',
          ),
          1 => 
          array (
            'name' => 'CELULAR',
            'label' => 'LBL_CELULAR',
            'customCode' => '{$CELULAR}',
          ),
        ),
        12 => 
        array (
          0 => 
          array (
            'name' => 'PASAPORTE',
            'label' => 'LBL_PASAPORTE',
            'customCode' => '{$PASAPORTE}',
          ),
          1 => 
          array (
            'name' => 'EMAIL',
            'label' => 'LBL_EMAIL',
            'customCode' => '{$EMAIL}',
          ),
        ),
      ),
      'lbl_detailview_panel2' => 
      array (
        0 => 
        array (
          0 => 
          array (
            'name' => 'consultation_fee',
            'label' => 'LBL_CONSULTATION_FEE',
          ),
          1 => 
          array (
            'name' => 'consultation_date',
            'label' => 'LBL_CONSULTATION_DATE',
          ),
        ),
        1 => 
        array (
          0 => 
          array (
            'name' => 'consultation_fee_to_company_c',
            'label' => 'LBL_CONSULTATION_FEE_TO_COMPANY',
          ),
          1 => '',
        ),
        2 => 
        array (
          0 => 
          array (
            'name' => 'migration_agent_name',
            'label' => 'LBL_MIGRATION_AGENT_NAME',
          ),
        ),
      ),
      'lbl_detailview_panel1' => 
      array (
        0 => 
        array (
          0 => 
          array (
            'name' => 'nvc_companies_veta_requerimiento_1_name',
            'label' => 'LBL_NVC_COMPANIES_VETA_REQUERIMIENTO_1_FROM_NVC_COMPANIES_TITLE',
          ),
        ),
        1 => 
        array (
          0 => 
          array (
            'name' => 'company_city',
            'label' => 'LBL_COMPANY_CITY',
          ),
          1 => 
          array (
            'name' => 'company_industry',
            'label' => 'LBL_COMPANY_INDUSTRY',
          ),
        ),
        2 => 
        array (
          0 => 
          array (
            'name' => 'email_company',
            'label' => 'LBL_EMAIL_COMPANY',
          ),
          1 => 
          array (
            'name' => 'phone_company',
            'label' => 'LBL_PHONE_COMPANY',
          ),
        ),
        3 => 
        array (
          0 => 
          array (
            'name' => 'company_sbs_expiry_date',
            'label' => 'LBL_COMPANY_SBS_EXPIRY_DATE',
          ),
          1 => 
          array (
            'name' => 'company_lead_id_c',
            'label' => 'LBL_COMPANY_LEAD_ID',
          ),
        ),
        4 => 
        array (
          0 => 
          array (
            'name' => 'company_turnover_c',
            'label' => 'LBL_COMPANY_TURNOVER',
          ),
          1 => '',
        ),
      ),
      'lbl_detailview_panel4' => 
      array (
        0 => 
        array (
          0 => 
          array (
            'name' => 'aplicant_company_mmm_fees',
            'label' => 'LBL_APLICANT_COMPANY_MMM_FEES',
          ),
          1 => 
          array (
            'name' => 'applicant_company_mmm_fees_without_gst_c',
            'label' => 'LBL_APPLICANT_COMPANY_MMM_FEES_WITHOUT_GST',
          ),
        ),
        1 => 
        array (
          0 => 
          array (
            'name' => 'applicant_company_total_c',
            'label' => 'LBL_APPLICANT_COMPANY_TOTAL',
          ),
          1 => 
          array (
            'name' => 'applicant_company_total_paid_c',
            'label' => 'LBL_APPLICANT_COMPANY_TOTAL_PAID',
          ),
        ),
        2 => 
        array (
          0 => 
          array (
            'name' => 'applicant_company_outstanding_c',
            'label' => 'LBL_APPLICANT_COMPANY_OUTSTANDING',
          ),
          1 => '',
        ),
      ),
      'lbl_detailview_panel9' => 
      array (
        0 => 
        array (
          0 => 
          array (
            'name' => 'marketing_question_1_c',
            'label' => 'LBL_MARKETING_QUESTION_1',
          ),
          1 => 
          array (
            'name' => 'marketing_question_2_c',
            'label' => 'LBL_MARKETING_QUESTION_2',
          ),
        ),
      ),
    ),
  ),
);
;
?>
