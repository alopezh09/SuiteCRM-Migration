...
class SOELHooksContacts {
    ...
    public function heredar_info( &$bean, $event, $arguments ) {
        ...
        $oportunidades = $bean->get_linked_beans('contacts_opportunities_1','Opportunities');
        foreach( $oportunidades as $o ) {
            ...
            foreach ($leads as $lead) {
                ...
                $q = "UPDATE leads SET first_name = '$bean->first_name',
                        last_name = '$bean->last_name',
                        phone_mobile = '$bean->phone_mobile',
                        phone_home = '$bean->phone_home'
                        WHERE id = '$lead->id'";

                $c->db->query( $q , "Error actualizando prospecto desde el contacto " . $bean->id);
                        
                $q = "UPDATE leads_cstm SET
                        pasaporte_c = '$bean->pasaporte_c',
                        visa_c = '$bean->visa_c',
                        pais_c = '$bean->pais_c',
                        departamento_c = '$bean->departamento_c',
                        ciudad_c = '$bean->ciudad_c'
                        WHERE id_c = '$lead->id'";
                $c->db->query( $q , "Error actualizando prospecto desde el contacto " . $bean->id);

                if($lead->id){
                    $q="UPDATE email_addresses
                    SET email_address = '$bean->email1',
                    email_address_caps = '".strtoupper($bean->email1)."'
                    WHERE id in (SELECT email_address_id
                    FROM email_addr_bean_rel 
                    where bean_id = '$lead->id' 
