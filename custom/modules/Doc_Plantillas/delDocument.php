...
class delDocumentoSolicitadoClass {
	...
	function delDocumentoSolicitado_Requeriments($bean, $event, $arguments){
		if ($_REQUEST[ 'module' ] == "Veta_Requerimiento") {
            
			$idRequeriment = $arguments['related_id'];
			$idPlantilla = $arguments['id'];
			
			$GLOBALS['log']-> error("el total es ".$idRequeriment);   
			$GLOBALS['log']-> error("Plantilla".$id);
	...
	where dpddc.doc_plantillas_doc_documentosdoc_plantillas_ida ='".$idPlantilla."'";
			...
			while($row = $GLOBALS['db']->fetchByAssoc($result) )
			{
				...
				$idDocumento = $row['doc_plantillas_doc_documentosdoc_documentos_idb'];
				$GLOBALS['log']-> error("Primero".$idDocumento);
				$sqlDocsSolicitados = "select ddddc.* from doc_docssolicitados_doc_documentos_c ddddc , veta_requerimiento_doc_docssolicitados_1_c ddoc 
	where ddddc.doc_docssolicitados_doc_documentosdoc_documentos_idb = '".$idDocumento."' and ddddc.doc_docssolicitados_doc_documentosdoc_docssolicitados_ida = ddoc.veta_requerimiento_doc_docssolicitados_1doc_docssolicitados_idb
	and ddoc.veta_requerimiento_doc_docssolicitados_1veta_requerimiento_ida ='".$idRequeriment."'";
				...
				while($rowDocsSolicitados = $GLOBALS['db']->fetchByAssoc($resultDocsSolicitados) )
				{
					...
					$GLOBALS['log']-> error("Segundo".$idDocSolicitado);
						...
						$queryInsertDocOpp = "UPDATE veta_requerimiento_doc_docssolicitados_1_c set deleted = 1 where veta_requerimiento_doc_docssolicitados_1doc_docssolicitados_idb ='".$idDocSolicitado."'";
						$GLOBALS['db']->query($queryInsertDocOpp);
