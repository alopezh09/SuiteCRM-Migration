...
class SugarWidgetSubPanelRejectButtonChecklist extends SugarWidgetField
{
	...
	*/
	...
	public function displayList(&$layout_def)
    {
		...
		$html ="<a id='reject_$record_id'>Reject</a>";
		$html .= "
		<script>
				$('#reject_$record_id').click(function(event) {
					event.preventDefault();  
					let val = $('#document_comment_c_$record_id').val();
					$.get('/reject_doc.php',{id:'$record_id',module:'$module_name',comment:val},function(responseText) { 
						location.reload() 
					});
				});
		</script>";
        // $html = "<a id=\"$unique_id\" onclick='return sp_del_conf();' href=\"javascript:sub_p_del('$subpanel', '$module_name', '$record_id', $refresh_page);\">"."Reject Item"."</a>";
        return $html;
    }
