	...
	public function updateDraftDoneDate(&$bean, $processStageField, $draftDoneField) {
		...
		if (empty($bean->$draftDoneField)) {
			...
			if (stripos($currentProcessStage, "Draft_done") === false && stripos($bean->$processStageField, "Draft_done") !== false) {
				...
				error_log("procesos_visas entro al Si, el valor del campo " . $bean->processStageField); 
    ...
    public function save( $check_notify = false ) {		
		...
		if(($this->estado != "Visa_Aplicada") and ($this->estado != "Visa_Granted_Notified") and ($this->estado != "Visa_Otorgada") and ($this->estado != "Visa_Negada")){
			...
			foreach ($fields as $processStageField => $draftDoneField) {
				if ($this->updateProcessLodgedStage($this, $processStageField, $draftDoneField)) {
					//$this->estado = "Process_Lodged";
					error_log("estado_visa dentro de la condicion de visas para asignar true a process_lodged_sw");
					$process_lodged_sw = true;
					break; // Si se actualiza el estado, no es necesario seguir verificando los demás campos
				}
			}
			...
			foreach ($fields as $processStageField => $draftDoneField) {
				if ($this->updateProcessGrantedStage($this, $processStageField, $draftDoneField)) {
					$this->estado = "Process_Granted";
					...
					if (isset($workflow)) {
						...
						if (isset($recibo)) {
							...
							foreach ($dr as $visa_subclass) {
								...
								if ((isset($curso->display_name_c)) && ($curso->display_name_c != '')) {
								...
								foreach ($fees as $fee) {
									error_log("Cantidad total hide_procesos FEES procesos tipo " . $fee->process_type_c);

									// Añadimos el proceso al array de tipos de procesos y aumentamos el contador de procesos
									$feesProcessTypes[] = trim($fee->process_type_c);
									$count_processes++; // Incrementamos el contador de procesos
									
									// Ahora verificamos el estado "granted" correspondiente en el objeto $this
									foreach ($fields as $processStageField => $grantedField) {
										
										error_log("Cantidad total validando procesos " . $processStageField);
										// Verificamos si el proceso corresponde con el campo adecuado
										if (strpos($processStageField, strtolower($fee->process_type_c)) !== false) {
											error_log("Cantidad total Verificando estado del campo: " . $processStageField . " - Valor actual: " . $this->$processStageField);
											...
											if (isset($this->$processStageField) && strpos($this->$processStageField, 'Granted') !== false) {
												...
												error_log("Cantidad total Proceso en estado granted: " . $fee->process_type_c);
											}
										}
									}
								}
			...
			error_log("Cantidad total de procesos asociados: " . $count_processes);
		...
		*/
		...
		if (($granted_count >= $count_processes) and ($count_processes > 0)){			
			if ($process_granted_sw){			
				$sc->estado = "Closed";	
				error_log("process_lodged Hold_Process_Lodge");			
				$save_sc = true;
			...
			$this->estado = "Closed";
			error_log("Cantidad total Estado: Closed");
		} else {
			error_log("Cantidad total Procesos Granted: " . $granted_count . " de " . $count_processes);
		}
