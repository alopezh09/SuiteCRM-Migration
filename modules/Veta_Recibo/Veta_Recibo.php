	...
	*/
	...
	private function set_consecutivo()
	{
		if (! isset($this->id) || empty($this->id)) {
			...
			$query = "SELECT MAX(CAST(name AS UNSIGNED)) AS num FROM veta_recibo WHERE deleted = 0";
			$result = $this->db->query($query, true);
			$row = $this->db->fetchByAssoc($result);
			
			if ($row != null) {
				$this->name = ((int)$row['num']) + 1;
			}
		}
	} 
    ...
    public function actualizar_oportunidad($o)
    {
        ...
        if ($o != null) {
            ...
            $this->consultation_fee_applicant = 0;
            $this->consultation_fee_company = 0;

            if ($req->consultation_fee_to_company_c == 0) {
                $this->consultation_fee_applicant = $req->consultation_fee;
            } else {
                $this->consultation_fee_company = $req->consultation_fee;
            }


            $total_without_gst = ($this->total_visabuclass_fees - $this->consultation_fee_applicant - $this->descuento) * 1;
            ...
            $company_total_without_gst = $this->total_visabuclass_fees_company - $this->company_discount_c - $this->consultation_fee_company;
    ...
    public function save($check_notify = false)
    {
		...
		// --- SETEAR invoice_company_creation_date_c SOLO AL CREAR (DATE) ---
		// --- SETEAR invoice_company_creation_date_c SOLO AL CREAR (DATE) ---
		if (empty($this->fetched_row) && empty($this->invoice_company_creation_date_c)) {
			$nowDb = $timedate->nowDb(); // YYYY-mm-dd HH:ii:ss (igual estilo date_entered)
			$this->invoice_company_creation_date_c = substr($nowDb, 0, 10); // YYYY-mm-dd (DATE)
		}
		
		// --- ACTUALIZAR FECHA DE CREACIÓN Y DEJAR REGISTRO EN AUDIT ---
		if (!empty($this->fetched_row)
			&& $this->invoice_creation_date_c != $this->fetched_row['invoice_creation_date_c']) {
			
			require_once 'include/utils.php';

			$beforeDateEntered = $this->fetched_row['date_entered'];

			// ⬇️ usar preferencia de fecha del usuario y pasarla a formato BD
			$userFormat = $current_user->getPreference('datef');              // ej: d/m/Y
			$dbDate     = $timedate->swap_formats(
				$this->invoice_creation_date_c,
				$userFormat,
				$timedate->get_db_date_format()                               // Y-m-d
			);

			// por si ya viene en formato BD o algo raro
			if (empty($dbDate)) {
				$dbDate = $timedate->to_db_date($this->invoice_creation_date_c, false);
			}

			$newDateTime = $dbDate . ' 00:00:00';                             // DATE → DATETIME

			// dejar el valor nuevo en el bean (Suite lo usará al hacer el UPDATE)
			$this->date_entered = $newDateTime;

			// actualizar directo en BD para asegurar
			$db->query(
				"UPDATE veta_recibo
				 SET date_entered = '" . $db->quote($newDateTime) . "'
			...
			$sqlAudit = "
				...
				VALUES (
					...
					'date_entered',
					'datetime',
					'" . $db->quote($beforeDateEntered) . "',
					'" . $db->quote($newDateTime) . "',
					NULL,
					NULL
				)
			";
		...
		*/
        ...
        if ($this->is_gerente_contable()) {
			...
			$this->sbs_amount_c = 0;
			$this->tas_amount_c = 0;
			$this->nomination_amount_c = 0;
			$this->skill_assessment_amount_c = 0;
			$this->state_nomination_amount_c = 0;
			$this->visa_amount_c = 0;
			
			$detalles = $this->get_linked_beans('veta_detallerecibo_veta_recibo', 'Veta_DetalleRecibo');

			// Reiniciar las variables de fees para esta ejecución
			foreach ($detalles as $d) {
				...
				// Crear un arreglo temporal para almacenar los montos asociados a cada process type
				$fees_accumulated = [];

				foreach ($fees_visasubclass as $fee_visa) {
					// Crear el nombre dinámico de la variable basado en el tipo de proceso
					$variable_name = strtolower($fee_visa->process_type_c) . '_amount_c';

					// Verificar si la propiedad dinámica existe antes de usarla
					if (property_exists($this, $variable_name)) {
						// Si la variable no ha sido inicializada para esta corrida, inicializarla
						if (!isset($fees_accumulated[$variable_name])) {
							$fees_accumulated[$variable_name] = 0;
						}

						// Acumular el valor del fee actual
						$fees_accumulated[$variable_name] += $fee_visa->fee;

						// Log para verificar qué está ocurriendo
						error_log("Process Type: " . $fee_visa->process_type_c . " | Fee: " . $fee_visa->fee);
						error_log("Variable Name: " . $variable_name . " | Accumulated Value: " . $fees_accumulated[$variable_name]);
					} else {
						// Si la propiedad no existe, omitir el proceso
						error_log("Process Type " . $fee_visa->process_type_c . " no tiene una variable asociada.");
					}
				}

				// Al final de la iteración, asignar los valores acumulados a las propiedades de la clase
				foreach ($fees_accumulated as $variable_name => $value) {
			...
			*/
            ...
            // Iterar sobre los campos de fees y verificar si tienen valores
            foreach ($fees_fields as $fee_field) {
                $fee_value = $this->$fee_field;

                error_log("Process Type department fee encontrado ");
                error_log("Process Type afuera del si ");

                if (!empty($fee_value)) {
                    error_log("Process Type esta por dentro del  del si ");
                    ...
                    error_log("Process Type department fee " . $fee_visa->name);

                    if (!empty($fee_visa->process_type_c)) {
                        // Crear el nombre dinámico de la variable basado en el tipo de proceso
                        $variable_name = strtolower($fee_visa->process_type_c) . '_amount_c';

                        error_log("Process Type department fee amount name " . $variable_name);
                        ...
                        if (property_exists($this, $variable_name)) {
                            ...
                            // Acumular el valor del fee actual
                            $fees_accumulated[$variable_name] += $fee_visa->total_visa;

                            // Log para verificar qué está ocurriendo
                            error_log("Process Type: " . $fee_visa->process_type_c . " | Fee: " . $fee_visa->total_visa);
                            error_log("Process Type Variable Name: " . $variable_name . " | Accumulated Value: " . $fees_accumulated[$variable_name]);
                        } else {
                            // Si la propiedad no existe, omitir el proceso
                            error_log("Process Type " . $fee_visa->process_type_c . " no tiene una variable asociada.");
                        }
                    } else {
                        error_log("Process Type El fee con id {$fee_value} no tiene process_type_c.");
                    }
                }
            }
    ...
    public function new_opportunity($recibo_id = false): Opportunity
    {
        ...
        if ($o == null) {
			...
			$o->visa_expiration_text_c = $person->expiracion_visa_c;
            ...
            if ($this_requirement->id != "") {
                ...
                if ($sw_assigned == 0) {
                    $o->user_id3_c = "2bc9f443-57c3-da50-6b91-61799ee28429";
                }

                $user = new User();
                $user->retrieve($o->user_id3_c);

                if ((isset($user->customer_services_user_c)) and ($user->customer_services_user_c != '')) {
                    $customer_service_user = $user->user_id_c;
                    error_log("ASIGNACION Entro usuarios asociados en el usuario");
                } else {
                    $customer_service_user = $this->get_customer_service_user();
                    error_log("ASIGNACION Entro asignacion automatica");
                }

                $o->user_id2_c = $customer_service_user;

                error_log("ASIGNACION id customer services " . $customer_service_user);
    ...
    public function new_recibo(Veta_Presupuesto $p): Veta_Recibo
    {
        ...
        if (isset($p->id) and ! empty($p->id)) {
            ...
            $companies = $p->get_linked_beans('nvc_companies_veta_presupuesto_1', 'Veta_Presupuesto');
            ...
            foreach ($companies as $company) {
                ...
                $r->load_relationship('nvc_companies_veta_recibo_1');
                $r->nvc_companies_veta_recibo_1->add($company->id);
            }
    ...
    public function update_totals($save = true)
    {
        ...
        if ($current_user->id === 'c5da6f79-e3de-2d8f-f42b-60d6505f50c2') {
            $training_plan = $this->get_training_plan_fee();
        }

        $total_without_gst = (($this->total_visabuclass_fees * 1) - ($this->consultation_fee_applicant * 1) - ($this->descuento * 1)) * 1;
        //error_log("total_without_gst -> SUMATORIA total_visabuclass_fees + consultation_fee + descuento - ". $total_without_gst);

        $total_taxes_GST = ($total_without_gst - $training_plan) * $taxes_GST; //$total_taxes_GST = ($this->total_visabuclass_fees - $req->consultation_fee - $this->descuento) * $taxes_GST;
		...
		$req->workplace_location_c = $this->workplace_location_c;
    ...
    public function update_cartera($save = true)
    {
		...
		*/
        ...
        $this->pendiente_por_pagar = max(($this->gran_total * 1) - ($this->pagado * 1)/* - ($this->get_refunds("Applicant") * 1)*/, 0);
        ...
        $this->company_outstanding_amount_c = max(($this->company_grand_total_c * 1) - ($this->company_paid * 1)/* - ($this->get_refunds("Company") * 1)*/, 0);
        ...
        $req->total_without_gst = ($this->total_visabuclass_fees - $this->consultation_fee_applicant - $this->descuento) * 1;
        ...
        $company_total_without_gst = ($this->total_visabuclass_fees_company - $this->company_discount_c - $this->consultation_fee_company) * 1;
        ...
        $req->applicant_company_mmm_fees_without_gst_c = ((($this->total_visabuclass_fees - $this->consultation_fee_applicant - $this->descuento) * 1) * 1) + ($req->company_total_without_gst_c * 1);
    ...
    function get_training_plan_fee()
    {
        // $detalles = $r->get_linked_beans('veta_detallerecibo_veta_recibo');
        $fee_fields = [
            'veta_tiposvisa_id1_c',
            'veta_tiposvisa_id2_c',
            'veta_tiposvisa_id3_c',
        ...
        foreach ($fee_fields as $fee_field) {
            $fee = BeanFactory::getBean('Veta_TiposVisa', $this->{$fee_field});
            logerror("TRAINING", $fee->name, $fee_field, $this->{$fee_field});
            if ($fee->training_plan_fee_c) {
                return $fee->training_plan_fee_c;
            }
        }
        return 0;
    }
