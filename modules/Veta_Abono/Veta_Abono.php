...
class Veta_Abono extends Basic
{
    ...
    private function set_consecutivo()
    {
        ...
        {
			$query  = "SELECT name as num FROM veta_abono order by date_entered desc limit 1";
    ...
    public function save( $check_notify = false )
    {
		...
		/*
		if(($this->custom_date_entered_applicant_c != '') or ($this->custom_date_entered_applicant_c )) {
			$this->date_entered = $this->custom_date_entered_applicant_c;
			error_log("custom date - Alfonso ". $this->custom_date_entered_applicant_c);
		}
		*/

        if ( $r->is_gerente_contable() )
        {
			// Solo ejecutar lógica de "primer abono" cuando es creación (no edición)
            $isNew = empty($this->fetched_row['id']);
    ...
    public function mark_deleted( $id )
    {
        ...
        if ( $rec->is_gerente_contable() )
        {
            ...
            foreach ( $recibos as $r )
				...
				$this->redireccionar_recibo( 'Deposit was Removed', $r->id );
        }
        else
        {
	...
	public function redireccionar_recibo( $msg, $registro )
    {
        if ( ! empty( $registro ) )
        {
            $aux = "<script>
