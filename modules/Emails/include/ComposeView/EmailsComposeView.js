...
(function ($) {
    ...
    self.construct = function () {
      ...
      if ($(self).find('#from_addr_name').length !== 0) {
        ...
        $.ajax({
		  ...
		  $('#email_to_applicant_c').click(function() {
			// Obtener el valor actual del campo
			var currentValue = $('#to_addrs_names').val();

			// Correo asociado al checkbox
			var email = $('#virtual_lead_email_c').text();

			if ($(this).prop('checked')) {
				if (currentValue === '') {
					// Si el campo está vacío, simplemente agregamos el correo
					$('#to_addrs_names').val(email);
				} else {
					// Si el campo no está vacío, agregamos el correo después de una coma
					$('#to_addrs_names').val(currentValue + ', ' + email);
				}
			} else {
				// Eliminamos el correo del campo
				var emails = currentValue.split(', ').filter(function(e) { return e !== email; });
				$('#to_addrs_names').val(emails.join(', '));
			}
		});

		$('#email_to_company_c').click(function() {
			var currentValue = $('#to_addrs_names').val();
			var email = $('#email_company').text();
  ...
  $.fn.EmailsComposeView.defaults = {
    "tinyMceOptions": {
	  ...
	  //toolbar: ['fontselect | fontsizeselect | formatselect | bold italic underline strikethrough | styleselect | numlist bullist outdent indent | alignleft aligncenter alignright alignjustify | removeformat'],	  	  
	  toolbar: ['fontselect | fontsizeselect | formatselect | bold italic underline strikethrough | styleselect | numlist bullist outdent indent | alignleft aligncenter alignright alignjustify | removeformat | link image | forecolor backcolor'],	  	
