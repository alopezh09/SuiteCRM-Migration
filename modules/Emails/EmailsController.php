...
class EmailsController extends SugarController
{
	...
	function rewrite_subject()
	{
		$module_dict = [
			"Opportunities" => "WRK",
			"Veta_Presupuesto" => "BDG",
			"Veta_Recibo" => "BLL",
			"Veta_Requerimiento" => "REQ",
			"Veta_Aplicacion" => "APP",
			"Veta_ServicioCliente" => "CUS"
		];
		$regex = "/\[(" . implode("|", array_values($module_dict)) . ") - \d+\]/i";

		$module = $module_dict[$this->bean->parent_type];
		$name = $this->bean->parent_name;

		if ($this->bean->parent_type == 'Leads') {
			$module = $module_dict['Veta_Requerimiento'];
			$lead = new Lead();
			$lead->retrieve($this->bean->parent_id);
			$r = $lead->get_linked_beans('veta_requerimiento_leads', 'Veta_Requerimiento');

			if (!count($r)) return;

			$requerimiento = $r[0];
			foreach ($r as $req) {
				if ($req->estado !== 'Descartado' && $requerimiento->date_entered < $req->date_entered) {
					$requerimiento = $req;
				}
			}
			$name = $requerimiento->name;
		}
		$this->bean->name = preg_replace($regex, '', $this->bean->name)  . " [$module - $name]";
		return;
	}

	public function answer_ticket()
	{
		$replyEmail =  BeanFactory::getBean('Emails', $_REQUEST['record']);
		$tickets = $replyEmail->get_linked_beans('auto_tickets_emails');
	...
	public function add_to_module($ticket)
	{
		if(empty($ticket)) return;
		foreach (Auto_Tickets::$module_dict as $relationshipDef) {
			$beans = $ticket->get_linked_beans($relationshipDef['relationship']);
			if (count($beans)) {
				$this->bean->parent_id = $beans[0]->id;
				$this->bean->parent_type = $beans[0]->module_name;
				return;
			}
		}
	}
	...
	public function action_send()
	{
		...
		if ($this->userIsAllowedToSendEmail($current_user, $inboundEmailAccount, $this->bean)) {
			...
			if ($this->bean->send()) {
                ...
                $this->bean->save();
				if ($_POST['refer_action'] == 'ReplyTo' || $_POST['refer_action'] ==  'ReplyToAll') {
					$this->add_to_module($this->answer_ticket());
				}
			} else {
	...
	{
        $UIDs = explode(',', $this->getRequestedUIDs($_REQUEST));
        ...
        foreach ($UIDs as $uid) {
            ...
            $this->sendRequest('http://localhost:3030/emails/delete', $requestData);
            $email->deleted = 1;
            $email->save();
		}
        echo json_encode(array('response' => true));
	...
	public function markEmails($request)
	{
        ...
        $UIDs = explode(',', $this->getRequestedUIDs($request));
        ...
        foreach ($UIDs as $uid) {
            ...
            switch ($type) {
                case 'unread':
                    $this->sendRequest('http://localhost:3030/emails/unread', $requestData);
                    break;
                case 'read':
                    $this->sendRequest('http://localhost:3030/emails/read', $requestData);
                    break;
