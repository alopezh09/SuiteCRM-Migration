...
class Email extends Basic
{
    ...
    public function handleBodyInHTMLformat($mail)
    {
	    ...
	    $styledBody = '<span style="color: #7030A0; font-size: 11.0pt; font-family: \'Calibri\', sans-serif;">' . $body . '</span>';
		$mail->Body = $styledBody;
		
		//error_log("BODY EMAIL CARGADO4". $body);
		error_log( "BODY EMAIL CARGADO44 ". print_r($_REQUEST, TRUE) );
		...
		*/
		...
		if($_REQUEST["parent_type"] == 'Opportunities'){
			$o = new Opportunity();
			...
			$req->retrieve($recibo->veta_requerimiento_veta_reciboveta_requerimiento_ida);
			
			$comp = new NVC_Companies();
			$comp->retrieve($req->nvc_companies_veta_requerimiento_1nvc_companies_ida);
			
			error_log("BODY_COMPANIA ID - ". $req->nvc_companies_veta_requerimiento_1nvc_companies_ida);
			
			
			error_log("BODY Proceso de ventas ". $o->name);
			...
			$docs_additionals = $o->get_linked_beans('doc_documentos_adic_opportunities', 'Doc_Documentos_Adic');
			
			/*
			Variables de Email templates
				$checklist_docs_requested_applicant 
				$checklist_additional_docs_applicant
			*/
			$docs_requested_applicant = '<ul>';
			$docs_requested_company = '<ul>';
			foreach ($docs_solicitados as $documento) {	

				if(!$documento->internal_document_c){
					if($documento->requested_to_c != 'Company'){
						$docs_requested_applicant = $docs_requested_applicant.'<li>'.$documento->name.'</li>';
					} else {
						$docs_requested_company = $docs_requested_company.'<li>'.$documento->name.'</li>';
					}
				}
			}
			
			foreach ($docs_additionals as $documento) { 
				if(!$documento->internal_document_c){
					if($documento->requested_to_c != 'Company'){
						$docs_requested_applicant = $docs_requested_applicant.'<li>'.$documento->name.'</li>';
					} else {
			...
			$docs_requested_applicant = $docs_requested_applicant.'</ul>';
			$docs_requested_company = $docs_requested_company.'</ul>';
			
			$u = new User();
			$u->retrieve($o->user_id3_c); 
			...
			error_log("BODY EMPLEADO NOMBRE". $u->name . "EMAIL " . $e->email1);
			//Employees
			
			$mail->Body = str_replace("$" . "checklist_docs_requested_applicant", $docs_requested_applicant, $mail->Body);
			$mail->Body = str_replace("$" . "checklist_docs_requested_company", $docs_requested_company, $mail->Body);
			$mail->Body = str_replace("$" . "case_manager_name_email", $e->email1, $mail->Body);
			$mail->Body = str_replace("$" . "case_manager_name", $o->case_manager_c, $mail->Body);
			$mail->Body = str_replace("$" . "applicant_name", $o->leads_opportunities_1_name, $mail->Body);
			
			$mail->Body = str_replace("$" . "company_email", $comp->email_docs_portal_c, $mail->Body);
			
			error_log("Email_Color " . $mail->Body);
			
			
			
			$customerJourneyURL = '';
			$Customer_Journey_URL = '';
			
			//if($o->name == "233"){
				error_log("CUSTOMER JOURNEY - WORKFLOW " . $o->name);
				...
				if ((isset($curso->subclass_c)) and ($curso->subclass_c != '')) {			
					error_log("VISASUBCLASS - VISA SUBCLASS NAME " . $visaSubClassName);
					$visaSubclassNumber = $curso->subclass_c;
						
					
				}
				
				
				switch ($curso->subclass_c) {
					case  '186':
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/ETaDucEkOddIi5WiwgIFBAEBD9eWMuVuPf48eMH3ydd3Dw'>Customer Journey</a>";
						break;

					case  '189':
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/EaTkji4w3I1HqEVgJ7G5-H4BPPuI19pMyDWIJCX4qiZ75g'>Customer Journey</a>";
						break;
					
					case  '190':
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/EVl9IpHsBe9NgB9G1w80YrsBKmquBZi22yJ_-o14laRRMw'>Customer Journey</a>";
						break;
					...
					case  '407':
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/EUzCQ0oUA4NJrxSkc651XRkB2EdNn6hNS0pCeoZinh-dlA'>Customer Journey</a>";
						break;
						
					case  '482':
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/EcjP6Ztc0ydNroBWdSYyyzABO-8Akb89UcZ0ijrMsCVIbg'>Customer Journey</a>";
						break;
					
					case  '494':
						if ($curso->tipovisa == 'DAMA'){
							$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/Eb0zzIr6BKNOtqydqrQEDGwBpoavNjD1pqAWGMOMlXUV6w'>Customer Journey</a>";
						}
					/*	
						if REGIONAL
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/r/personal/admin_mmmveta_onmicrosoft_com/Documents/Customer%20Journey%20MMM/Customer%20Journey_494%20VISA.pdf?csf=1&web=1&e=GN7z5t'>Customer Journey</a>";
					*/	
						break;
					

					default:						
						$Customer_Journey_URL = "<a target='_blank' href='https://mmmveta-my.sharepoint.com/:b:/g/personal/admin_mmmveta_onmicrosoft_com/EdqM_EIvGYJChCH004W85d4Bci017XFg-Ibbt5xautnOMw'>Customer Journey</a>";						
						break;
				}
				 
				$mail->Body = str_replace("$" . "Customer_Journey_URL", $Customer_Journey_URL, $mail->Body);
				
				error_log("CUSTOMER JOURNEY - VISASUBCLASS NUMBER " . $visaSubclassNumber); 
				error_log("CUSTOMER JOURNEY - VISASUBCLASS NAME " . $visaSubClassName); 
			...
			/*
			$i = new Veta_Recibo();
			$invoices = $o->get_linked_beans('veta_recibo_opportunities', 'Opportunities');			
			...
			*/
			...
			if( $_REQUEST["emails_email_templates_idb"] == "97ca24bd-9773-d7af-68e0-638915df50fb"){
				
				$change_status = 1;
				
				$query = "select checklist_sent_date_c from opportunities_cstm where id_c = '" . $o->id . "'";
				$result = $o->db->query($query, true, " Error filling in additional list fields: ");
				$row = $o->db->fetchByAssoc($result);

				if ((isset($row['checklist_sent_date_c'])) and ($row['checklist_sent_date_c'] != '')) {					
					$change_status = 0;
				}
				...
				$o->db->query("UPDATE 
					opportunities_cstm 
				SET 
					checklist_sent_date_c = '" . date('Y-m-d H:i:s') . "'
					/*estado_servicio_al_cliente_c = 'Checklist_Consultation_Sent'*/
				WHERE 
					id_c = '" . $o->id . "'");			
				...
				$invoices = $o->get_linked_beans('veta_recibo_opportunities', 'Opportunities');			
				foreach ($invoices as $invoice) {	
					$requirements = $invoice->get_linked_beans('veta_requerimiento_veta_recibo', 'Veta_Recibo');			
					foreach ($requirements as $requirement) {								
						$mail->Body = str_replace("$" . "migration_manager_user", $requirement->migration_manager_user_c, $mail->Body);
						$mail->Body = str_replace("$" . "migration_manager_password", $requirement->migration_manager_password_c, $mail->Body);
						
						error_log("BODY requirement requirement". $requirement->name);
					}
				}
			} else if( $_REQUEST["emails_email_templates_idb"] == "de716899-56a8-be85-18bf-639121c82d0a"){
				$change_status = 1;
				$query = "select company_checklist_sent_date_c from opportunities_cstm where id_c = '" . $o->id . "'";
				$result = $o->db->query($query, true, " Error filling in additional list fields: ");
				$row = $o->db->fetchByAssoc($result);

				if ((isset($row['company_checklist_sent_date_c'])) and ($row['company_checklist_sent_date_c'] != '')) {					
					$change_status = 0;
				}
				...
				$o->db->query("UPDATE 
					opportunities_cstm 
				SET 
					company_checklist_sent_date_c = '" . date('Y-m-d H:i:s') . "'
					/*estado_servicio_al_cliente_c = 'Checklist_Consultation_Sent'*/
				WHERE 
				id_c = '" . $o->id . "'");	
				...
				$invoices = $o->get_linked_beans('veta_recibo_opportunities', 'Opportunities');			
				foreach ($invoices as $invoice) {	
					$detalles = $invoice->get_linked_beans('veta_detallerecibo_veta_recibo', 'Veta_DetalleRecibo');
					foreach ($detalles as $detalle) {	
						$mail->Body = str_replace("$" . "visa_subclass", $detalle->name, $mail->Body);						
							
						error_log("BODY DETALLE RECIBO". $detalle->name);
					}
				}
			}
			
			if($change_status == 1){
				//$cs = new Veta_ServicioCliente();
				$css = $o->get_linked_beans('veta_serviciocliente_opportunities', 'Veta_ServicioCliente'); 
				
				foreach ($css as $cs) {
					$cs->estado = 'Checklist_Consultation_Sent';				
					error_log("BODY EMPLEADO NOMBRE". $cs->name . "CUSTOMR SERVICES " . $cs->id); 
					$cs->save();
				}
			}
			...
			//$mail->Body = str_replace("$" . "$checklist_additional_docs_applicant", $docs_requested_applicant, $mail->Body);
		}
    ...
    public function send(
		...
		error_log("VALIDATION tiempo de ejecucion INICIO FUNCTION ". date("d/m/Y, h:i:s"));
        ...
        foreach ((array)$this->to_addrs_arr as $addr_arr) {
			...
			error_log("VALIDATION tiempo de ejecucion FOREACH PRIMERO - LEE CUENTAS DE CORREO". date("d/m/Y, h:i:s"));
        }
        ...
        foreach ((array)$this->cc_addrs_arr as $addr_arr) {
			...
			error_log("VALIDATION tiempo de ejecucion FOREACH PRIMERO - LEE CC". date("d/m/Y, h:i:s"));
        }
        ...
        foreach ((array)$this->bcc_addrs_arr as $addr_arr) {
			...
			error_log("VALIDATION tiempo de ejecucion FOREACH PRIMERO - LEE BCC ". date("d/m/Y, h:i:s"));
        }
        ...
        if (isset($this->saved_attachments)) {		
			...
			// Array para almacenar adjuntos únicos
			$uniqueAttachments = array(); 
			// Iterar sobre los adjuntos guardados
			foreach ($this->saved_attachments as $note) {
			  // Generar un hash único para este adjunto
			  $hash = md5($note->id . $note->filename);
			  // Verificar si ya fue agregado previamente
			  if (!in_array($hash, $uniqueAttachments)) {
				// Si es nuevo, agregar al array
				$uniqueAttachments[] = $hash;
				// Agregar a la lista final de adjuntos únicos
				$uniqueSavedAttachments[] = $note;
			  }
			}
			// Sobrescribir con los adjuntos únicos
			$this->saved_attachments = $uniqueSavedAttachments;			
			
			//error_log("adjunto - CONTEO " .  count($this->saved_attachments) );
            foreach ($this->saved_attachments as $note) {
				
				error_log("adjunto - primer adjunto " .  print_r($note, true) );
				...
				//error_log("adjunto - primer adjunto " .  print_r($mail, true) );
				error_log("adjunto - primer adjunto2 ");
        ...
        if ($mail->send()) {
            ...
            // mark replied
            $this->outlook_api_id_c = $mail->outlook_api_id_c;
            ...
            ///////////////////////////////////////////////////////////////////
			error_log("VALIDATION tiempo de ejecucion FINAL DEL ENVIO DE LA FUNCTION - return TRUE  ". date("d/m/Y, h:i:s"));
            return true;
        }
        $GLOBALS['log']->debug($app_strings['LBL_EMAIL_ERROR_PREPEND'] . $mail->ErrorInfo);

		error_log("VALIDATION tiempo de ejecucion FINAL DEL ENVIO DE LA FUNCTION - return false  ". date("d/m/Y, h:i:s"));
