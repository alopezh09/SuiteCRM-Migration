...
{
    public const REQUERIMIENTOINFOCOMPLETADO = 'Cannot create quote because status is Info or Completed';
    public const REQUERIMIENTOSINREFERIDO = 'The budget cannot be created because the requirement does not have a referral';
    public const REQUERIMIENTOSINFUENTE = 'The budget cannot be created because the requirement has no source';
    public const PROSPECTOSINEMAIL = 'The budget cannot be created because the prospect does not have an email';
    public const PROSPECTOSINCELULAR = 'The budget cannot be created because the prospect does not have a cell phone';
    public const RECIBOCREADONOPRESUPUESTO = 'It is not possible to save the budget because an invoice has already been created';
    ...
    public function save( $check_notify = false )
    {
		...
		$recibos = $requerimiento->get_linked_beans('veta_requerimiento_veta_recibo', 'Veta_Recibo');

        if ( count( $recibos ) == 0 or 1==1) {
            ...
            $department_visa_fee_1 -> retrieve($this->veta_tiposvisa_id2_c);
            $department_visa_fee_2 = new Veta_TiposVisa();
            $department_visa_fee_2 -> retrieve($this->veta_tiposvisa_id3_c);
            $department_visa_fee_3 = new Veta_TiposVisa();
            $department_visa_fee_3 -> retrieve($this->veta_tiposvisa_id4_c);
            $department_visa_fee_4 = new Veta_TiposVisa();
            $department_visa_fee_4 -> retrieve($this->veta_tiposvisa_id5_c);
            $department_visa_fee_5 = new Veta_TiposVisa();
            $department_visa_fee_5 -> retrieve($this->veta_tiposvisa_id6_c); 
			$department_visa_fee_6 = new Veta_TiposVisa();
			$department_visa_fee_6 -> retrieve($this->veta_tiposvisa_id11_c); 
			$department_visa_fee_7 = new Veta_TiposVisa();
			$department_visa_fee_7 -> retrieve($this->veta_tiposvisa_id12_c);
            ...
            $department_visa_fee_c_1 -> retrieve($this->veta_tiposvisa_id7_c);
            $department_visa_fee_c_2 = new Veta_TiposVisa();
            $department_visa_fee_c_2 -> retrieve($this->veta_tiposvisa_id8_c);
            $department_visa_fee_c_3 = new Veta_TiposVisa();
            $department_visa_fee_c_3 -> retrieve($this->veta_tiposvisa_id9_c);
            $department_visa_fee_c_4 = new Veta_TiposVisa();
            $department_visa_fee_c_4 -> retrieve($this->veta_tiposvisa_id10_c);
     ...
     */
    public function crear_relaciones()
    {
        ...
        foreach ( $requirements as $req ) {
			...
			$company = $req->get_linked_beans( 'nvc_companies_veta_requerimiento_1', 'Veta_Requerimiento' );

            foreach ( $company as $com ) {

                $com->load_relationship( 'nvc_companies_veta_presupuesto_1' );
                $com->nvc_companies_veta_presupuesto_1->add( $this->id );
            }			
     ...
     */
    public function establecer_primer_presupuesto()
    {
        if ( $_REQUEST[ "relate_to" ] == 'veta_requerimiento_veta_presupuesto' ) {
    ...
    public function update_totals()
    {
		...
		$r->aplicant_first_payment_amount = number_format(($this->gran_total), 2,",",".");
        $r->aplicant_mmm_fee = number_format(($total_visabuclass_fees_GST), 2,",",".");
        $r->aplicant_company_mmm_fees = number_format(($total_visabuclass_fees_GST + $company_mmm_fee), 2,",",".");
        $r->Total = number_format(($this->gran_total), 2,",",".");

        $r->company_mmm_fee = number_format(($company_mmm_fee), 2,",",".");
        $r->company_first_payment_amount = number_format(($company_first_payment_amount), 2,",",".");
        $r->company_mmm_fee = number_format(($this->total_visabuclass_fees_company), 2,",",".");
        $r->company_total_without_gst_c = number_format(($company_total_without_gst), 2,",",".");
        ...
        if ($r->total_paid == 0 ) {
            $r->total_paid = number_format((0), 2,",",".");
        }        
        $r->consultation_fee = number_format(($r->consultation_fee), 2,",",".");
        $r->discount = number_format(($this->descuento), 2,",",".");

        

        $r->total_without_gst = number_format(($total_without_gst), 2,",",".");
        $r->gst_percentage = number_format(($total_taxes_GST), 2,",",".");
        $r->department_visa_fee_base_application_charge = number_format(($this->total_visa), 2,",",".");
        $r->department_credit_card_surcharge_percentage = number_format(($total_taxes_Department_Credit_Card), 2,",",".");
        $r->insurance_value = number_format(($this->seguro), 2,",",".");       
