...
class Veta_PresupuestoViewSend extends ViewEdit
{
	...
	function preDisplay()
	{
		...
		if ($o != null) {
			...
			//$this->redireccionar($tEmail->name, $p->id);
			...
			$mail = Media::prepare_email_from_template(NULL, [$o->email1], 'presupuesto', [
				'$cliente' => $o->name,
			...
			if ($mail->send()) {
				...
				if ($p->send_both_quote_to_applicant_c) {
					//$this->redireccionar("valor true", $p->id);
					$toemail = null;
					...
					if ($toemail != null) {
						...
						$tEmail->body_html = str_replace("$" . "assigned_user_name", $u->name, $tEmail->body_html);
						$tEmail->subject = str_replace("$" . "append", '', $tEmail->subject);
						...
						], ['$append' => $tEmail->subject]);


						$mail->addAttachment($sugar_config['upload_dir'] . $p->id . '.pdf',  $file_name, 'base64', 'application/pdf');
						if ($mail->send()) {
							...
							// $nota = $this->crear_nota($p, $emailObj);
							$emailObj = crear_email($p, $mail, true);
					...
					$this->redireccionar('Email have been sent to Applicant ' . $o->email1, $p->id);
				}
			} else {
				$this->redireccionar('No fue posible enviar el email por favor revisa la configuración de correo de tu cuenta', $p->id);
			}
