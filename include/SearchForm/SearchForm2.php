...
class SearchForm
{
    ...
    public function display($header = true)
    {
        ...
        $totalWidth = 0;
        if (
            isset($this->searchdefs['templateMeta']['widths'])
    ...
    */
    public function _build_field_defs()
    {
        ...
        if ($this->seed) {
            ...
            foreach ($this->seed->toArray() as $name => $value) {
                ...
                if (
                    isset($this->fieldDefs[$name]['type']) && $this->fieldDefs[$fvName]['type'] == 'function'
                    ...
                    foreach ($this->seed->field_name_map as $key => $params) {
                        if ($key != 'assigned_user_name' && $key != 'modified_by_name') {
                            ...
                            if (in_array($key . '_' . $SearchName, $arrayKeys) && !in_array($key, $searchFieldsKeys)) {
                                ...
                                if (
                                    !empty($params['type']) && $params['type'] == 'parent'
    ...
    public function generateSearchWhere($add_custom_fields = false, $module = '')
    {
        ...
        foreach ((array)$this->searchFields as $field => $parms) {
            ...
            if (
                !empty($this->seed->field_name_map[$field]['source'])
            ...
            if (isset($parms['value']) && $parms['value'] != "") {
                ...
                if (is_array($parms['value'])) {
                    ...
                    if (!empty($this->seed->field_name_map[$field]['isMultiSelect']) && $this->seed->field_name_map[$field]['isMultiSelect']) {
                        ...
                        if (
                            isset($this->seed->field_name_map[$field]) &&
                ...
                if ($field_value != '' || $operator == 'isnull') {
                    ...
                    foreach ($parms['db_field'] as $db_field) {
                        if (strstr($db_field, '.') === false) {
                            ...
                            if (
                                $type == 'relate' && !empty($this->seed->field_name_map[$field]['link'])
                        ...
                        switch (strtolower($operator)) {
                            ...
                            case 'today':
                                ...
                                global $timedate;
                                $today_date = $timedate->nowDbDate();
                                $tomorrow_date = $timedate->getNow()->modify("+1 day")->asDbDate();
                                $yesterday_date = $timedate->getNow()->modify("-1 day")->asDbDate();
                                /*
                                    $where .= "{$db_field} > '$today_date 00:00:00' and {$db_field} <= '$today_date 23:59:59'";
                                    break;
                                    */
                                // Verifica el tipo de campo
                                if ($this->seed->field_name_map[$field]['type'] == 'datetime') {
                                    // Caso para datetime
                                    $where .= "{$db_field} > '$today_date 00:00:00' and {$db_field} <= '$today_date 23:59:59'";
                                } else {
                                    // Caso para date
                                    $where .= "{$db_field} = '{$today_date}'";
                                }
                                break;
