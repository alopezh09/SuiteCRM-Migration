...
eoq;
	...
	public function PreSend()
	{
		...
		logerror("email_prueba",$this->Body);
		$aux = parent::preSend();
		logerror("email_prueba",$this->Body);
		return $aux;
	}
	...
	public function queueEmail()
	{
		...
		// if ($current_user->id !== 'c5da6f79-e3de-2d8f-f42b-60d6505f50c2' && $current_user->id !== 'af54b5d9-e7ce-2047-b1c5-6243340c1ab6')
		// 	return parent::send();
		logerror($this->attachment);
		$pattern = '/[a-z0-9_\-\+\.]+@[a-z0-9\-]+\.([a-z]{2,4})(?:\.[a-z]{2})?/i';
		preg_match_all($pattern, $this->From, $matches);
		$data = [
			'from' => [
				'addr' => $matches[0][0],
				'name' => $this->FromName
			...
			'cc' => $this->cc,
			'bcc' => $this->bcc,
			'subject' => mb_convert_encoding($this->Subject, 'UTF-8', 'UTF-8'),
			'body' => mb_convert_encoding($this->Body, 'UTF-8', 'UTF-8'),
			'attachments' => array_map(function ($attachment) use ($sugar_config) {
				logerror($attachment);
				return [
					"path" => $sugar_config['upload_dir'] . $attachment[1],
					"filename" => iconv_mime_decode($attachment[2])
				];
			}, $this->attachment),
			'time' => time(),
			'sent' => false
		...
		$filename = md5(time());
		logerror("email_prueba",$filename);

		$this->outlook_api_id_c = $filename;
		if (file_put_contents("/home/ubuntu/outlook-api/files/$filename", json_encode($data)) === false)
			return parent::send();



		if (chmod("/home/ubuntu/outlook-api/files/$filename", 0777) === false)
			return parent::send();
	...
	public function send()
	{
		...
		error_log("VALIDATION INICIO FUNCION EN SUGARPHPMAILER  " . date("d/m/Y, h:i:s"));
		...
		try {
			...
			error_log("VALIDATION DENTRO DEL TRY EN SUGARPHPMAILER  " . date("d/m/Y, h:i:s"));
		} catch (Exception $e) {
